...
SELECT c.num_cadastro AS num_cliente,
...
SELECT e.nome AS nome_estrela,
       e.nome_real AS nome_real,
	   f.titulo AS titulo_filme
FROM filme f INNER JOIN filme_estrela fe
ON  f.id = fe.id
INNER JOIN estrela e
ON fe.id_estrela = e.id_estrela
WHERE f.ano = 2015
...
SELECT CONVERT(VARCHAR(10), d.data_fabricacao, 103) AS data_fabricacao,
	   f.titulo AS nome_filme,
	   f.ano AS ano_filme,
	  CASE 
        WHEN (YEAR(GETDATE()) - f.ano) >= 6 THEN 
            CONVERT(VARCHAR, (YEAR(GETDATE()) - f.ano)) + ' anos'
        ELSE 
            CONVERT(VARCHAR, (YEAR(GETDATE()) - f.ano))
    END AS diferenca_ano
FROM filme f INNER JOIN dvd d
ON f.id = d.id
...
SELECT c.num_cadastro AS num_cliente,
       c.nome AS nome_cliente,
	   MAX(CONVERT(VARCHAR(10), d.data_fabricacao, 103)) AS data_fabricacao,
	   f.titulo AS nome_filme,
	   l.valor
FROM cliente c INNER JOIN locacao l
ON c.num_cadastro = l.num_cadastro
...
INNER JOIN filme f
ON d.id = f.id
GROUP BY c.num_cadastro, c.nome, f.titulo, l.valor
...
SELECT c.num_cadastro AS num_cliente,
       c.nome AS nome_cliente,
	   CONVERT(VARCHAR(10), l.data_locacao, 103) AS data_locacao,
	   COUNT(l.data_locacao) AS qtd_dvd_alugado
FROM cliente c INNER JOIN locacao l
ON c.num_cadastro = l.num_cadastro
INNER JOIN dvd d
ON l.num = d.num
...
SELECT c.num_cadastro AS num_cliente,
       c.nome AS nome_cliente,
	   c.logradouro +' ' + CAST(c.num AS VARCHAR(10)) AS endereco,
	   CONVERT(VARCHAR(10), l.data_locacao, 103) AS data_locacao
FROM cliente c INNER JOIN locacao l
ON c.num_cadastro = l.num_cadastro
WHERE
